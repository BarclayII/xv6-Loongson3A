...
\begin{document}
	...
	everything, \texttt{mpmain} calls \texttt{scheduler} to start running processes (\hilite{NYI}). 
	\texttt{Scheduler} (\hilite{NYI})
	...
	to the process it found and \hilite{
		...
		register
	}. switchuvm also sets up a task
	state segment SEG\_TSS that instructs the hardware to execute system calls and inter-
	rupts on the process’s kernel stack. We will re-examine the task state segment in
	Chapter 3.
